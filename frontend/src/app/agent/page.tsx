...
export default function AgentDashboard() {
  ...
  const updateAgentStatus = async (newStatus: string) => {
    ...
    try {
      await api.patch('/agents/availability', { status: newStatus });
    ...
    <Layout>
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Welcome, {profile?.user?.full_name?.split(' ')[0] || 'Agent'}!</h1>
            <p className="text-gray-600">{profile?.employee_id || 'N/A'} • {profile?.status ? profile.status.charAt(0).toUpperCase() + profile.status.slice(1) : 'N/A'}</p>
          </div>
          <select value={profile?.status || 'offline'} disabled={isUpdatingStatus} onChange={(e) => updateAgentStatus(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 disabled:opacity-60">
