...
type QuoteBreakdown = {
  volumetric_weight_kg: number; billable_weight_kg: number; zone_type: string;
  base_charge: number; cod_surcharge: number; total_charge: number;
  applied_rule?: string; applied_cod_surcharge?: string;
};
type QuoteResult = { success: boolean; breakdown?: QuoteBreakdown; pickup_zone?: { id: string; name: string; code: string }; drop_zone?: { id: string; name: string; code: string }; error?: string };
...
export default function CreateOrderPage() {
  ...
  const getQuote = async () => {
    ...
    catch (err: any) { const detail = err.response?.data?.detail; setError(typeof detail === 'string' ? detail : 'Unable to calculate the quote.'); }
  ...
  const onSubmit = async (data: OrderForm) => {
    ...
    catch (err: any) { const detail = err.response?.data?.detail; setError(typeof detail === 'string' ? detail : 'Failed to create order. Please try again.'); }
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6"><div className="flex items-center justify-between gap-4"><div><h2 className="text-lg font-semibold text-gray-900 flex items-center"><DollarSign className="h-5 w-5 text-primary-600 mr-2" /> Price Quote</h2><p className="text-sm text-gray-500 mt-1">Calculate the shipping charge before placing the order.</p></div><button type="button" onClick={getQuote} disabled={isGettingQuote} className="btn-primary whitespace-nowrap">{isGettingQuote ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Calculating...</> : 'Get Quote'}</button></div>
            {quote?.success && quote.breakdown && <div className="mt-6 rounded-lg border border-green-100 bg-green-50 p-5"><div className="flex items-center text-green-700 font-medium mb-4"><CheckCircle className="h-5 w-5 mr-2" /> Quote calculated successfully</div><div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm"><div><p className="text-gray-500">Zone</p><p className="font-semibold text-gray-900">{quote.breakdown.zone_type}</p></div><div><p className="text-gray-500">Billable Weight</p><p className="font-semibold text-gray-900">{quote.breakdown.billable_weight_kg} kg</p></div><div><p className="text-gray-500">Base Charge</p><p className="font-semibold text-gray-900">{formatCurrency(quote.breakdown.base_charge)}</p></div><div><p className="text-gray-500">Total</p><p className="font-bold text-green-700 text-lg">{formatCurrency(quote.breakdown.total_charge)}</p></div></div></div>}
            {quote && !quote.success && quote.error && <div className="mt-4 p-3 rounded-lg bg-red-50 text-red-700 text-sm">{quote.error}</div>}
          </section>
