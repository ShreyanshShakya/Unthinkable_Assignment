...
import { Package, Truck, MapPin, Clock, DollarSign, ArrowLeft, AlertCircle, CheckCircle, XCircle, Loader2, MapPin as MapPinIcon, Calendar, User, Phone, Shield, Scale } from 'lucide-react';
...
const statusDescriptions: Record<string, string> = {
  ...
  picked_up: 'The delivery agent has picked up your package.',
  in_transit: 'Your package is on the way to the destination.',
  out_for_delivery: 'The package is out for delivery and will arrive soon.',
...
export default function OrderDetailPage() {
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true);
    try {
      await api.patch(`/orders/${orderId}/status`, { status: 'cancelled', reason: 'Cancelled by customer' });
      router.refresh();
    } catch (err: any) {
    ...
    return (
      <Layout>
        <div className="text-center py-12">
          ...
          <Link href="/dashboard" className="mt-4 inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700">
            Back to Dashboard
          </Link>
        </div>
      </Layout>
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between">
          <Link href="/dashboard/orders" className="text-gray-500 hover:text-gray-700 mb-2 inline-flex items-center">
                    ...
                    <div className="ml-6 pt-1 flex-1">
                      ...
                      {historyEntry && (
                        <p className="text-xs text-gray-400 mt-1">
                          {historyEntry.actor_role === 'customer' ? 'By you' : historyEntry.actor_role === 'agent' ? 'By delivery agent' : 'By admin'}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {order.status === 'failed' && order.failure_reason && (
              <div className="bg-red-50 rounded-xl border border-red-200 p-6">
                ...
                <p className="text-sm text-red-600 mt-2">You can request a reschedule from the order actions.</p>
              </div>
            )}
          </div>
              ...
              <div className="space-y-2">
                {order.status === 'created' && (
                  <button
                    ...
                    disabled={cancelling}
                    className="w-full px-4 py-2 border border-red-300 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50 flex items-center justify-center"
                  >
                ...
                {order.status === 'failed' && (
                  <Link
                    ...
                    className="w-full inline-flex items-center justify-center px-4 py-2 border border-primary-300 rounded-lg text-sm font-medium text-primary-600 hover:bg-primary-50 transition-colors"
                  >
                    Request Reschedule
                  </Link>
                )}
